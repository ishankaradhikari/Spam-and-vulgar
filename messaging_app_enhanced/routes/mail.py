...
def _save_attachment(file_obj):
    """Save upload; return stored filename or None."""
...
def _render_folder(folder_name, user_id, template="mailbox.html"):
    ...
    # Determine which user column to filter by
    if folder_name == "sent":
        owner_col = "m.sender_id"
    else:
        owner_col = "m.recipient_id"
...
def compose():
    ...
    if request.method == "POST":
        ...
        att_filename = _save_attachment(attachment)
        ...
        # If root message, set thread_id = its own id
        if not thread_id:
            db.execute("UPDATE messages SET thread_id=? WHERE id=?",
                       (recipient_msg_id, recipient_msg_id))

        # Insert sender copy (sent folder, always readable)
        db.execute(
            """INSERT INTO messages
               (thread_id, sender_id, recipient_id, subject, body, folder,
                is_read, attachment,
